...
const createMenu = (objetoPassadoNoCreateMenu) => {
  const objetoNovo = { fetchMenu: () => objetoPassadoNoCreateMenu };
  objetoNovo.consumption = [];
  objetoNovo.order = (consumivel) => {
    objetoNovo.consumption.push(consumivel);
  };
  objetoNovo.pay = () => {
    let price = 0;
    for (let index = 0; index < (objetoNovo.consumption).length; index += 1) {
      if (Object.keys(objetoNovo.fetchMenu().food).includes(objetoNovo.consumption[index])) {
        price += objetoNovo.fetchMenu().food[objetoNovo.consumption[index]];
      }
      if (Object.keys(objetoNovo.fetchMenu().drink).includes(objetoNovo.consumption[index])) {
        price += objetoNovo.fetchMenu().drink[objetoNovo.consumption[index]];
      }
    }
    price *= 1.1;
    return price;
